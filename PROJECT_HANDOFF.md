# 中壢課 自動排班系統 — 專案交接文件
> 建立日期：2026-06-10　最新版本：v14.4（Excel 匯出表頭＋尾部 3 欄）
...
| 主程式檔案 | `index.html`（單一檔案，約 4870 行） |
...
### 中優先
- [x] **Excel 匯出頭部重排**（左欄合併單欄、項目欄歸位、組別自動帶入，v14.3）
- [x] **Excel 匯出「尾」精修**（右側改 3 欄直式：排班／服勤／加班，v14.4）
- [x] **Excel 匯出表頭列**（每塊頂端「人員/店編號 ｜ 項目 ｜ 日期 ｜ 月標準工時」列，v14.4）
- [ ] **個人月工時資料來源**（匯出暫用實際服勤總時數；若需各員約定月時數配額，須新增 staff 屬性）
- [ ] **加班時數計算規則**（匯出加班欄目前留空手填，待定義自動計算公式）
