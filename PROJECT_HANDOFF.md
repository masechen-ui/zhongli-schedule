# 中壢課 自動排班系統 — 專案交接文件
> 建立日期：2026-06-10　最新版本：v14.3（Excel 匯出頭部重排）
...
| 主程式檔案 | `index.html`（單一檔案，約 4874 行） |
...
- [ ] **Excel 匯出「尾」精修**（右側統計區欄位重排 — 月標準工時／個人月工時／排班／服勤／加班／時數／公休／請假）
- [ ] **Excel 匯出表頭列**（補每塊頂端「人員/店編號 ｜ 項目」列，需同步調 `rowHeights` 的 `base=2+si*9`）
